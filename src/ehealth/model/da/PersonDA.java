...
public class PersonDA {
    ...
    //request #12
    public ArrayList<Person> selectByPerson_MeasureType_Min_Max(String measureName, double min, double max) throws Exception
    {
        statement = connection.prepareStatement(
                "SELECT p.idPerson, p.firstname, p.lastname, p.birthdate " +
                        ...
                        "AND temp.idMeasureDefinition = hmh.idMeasureDefinition AND temp.MaxDate = hmh.created " +
                        "AND hmh.value >= ? AND hmh.value <= ? " +
                        "INNER JOIN MeasureDefinition md ON md.idMeasureDef = hmh.idMeasureDefinition AND " +
                        "md.measureName = ? INNER JOIN Person p ON p.idPerson = hmh.idPerson");
        statement.setDouble(1, min);
        statement.setDouble(2, max);
        statement.setString(3, measureName);
        ...
        while (resultSet.next())
        {
            Person person = new Person();
            person.setIdPerson(resultSet.getLong("idPerson"));
            System.out.println(resultSet.getLong("idPerson"));
            person.setFirstname(resultSet.getString("firstname"));
            person.setLastname(resultSet.getString("lastname"));
            person.setBirthdate(resultSet.getDate("birthdate"));
            person.setMeasure(new HealthProfileDA().selectByPersonId(person.getIdPerson()));
